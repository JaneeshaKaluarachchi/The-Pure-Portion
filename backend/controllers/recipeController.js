...
const addRecipe = async (req, res) => {
  try {
    ...
    const recipeData = {
      ...
      pendingIngredients: req.body.pendingIngredients || [],
    };
    ...
    // Notify inventory manager when recipe has ingredients not yet in inventory
    const pending = recipeData.pendingIngredients || [];
    if (restaurantId && pending.length > 0) {
      try {
        await new Notification({
          type: 'recipe_added_missing_ingredients',
          title: `New recipe needs inventory update`,
          message: `Recipe "${recipe.name}" was added with ${pending.length} ingredient${pending.length > 1 ? 's' : ''} not found in inventory. Please add them to inventory ASAP so the recipe becomes available.`,
          fromModule: 'recipe_management',
          ...
          relatedData: {
            ...
            missingIngredients: pending.map((p) => ({
              name: p.name,
              quantity: p.quantity,
              unit: p.unit,
            })),
        ...
        console.error('Failed to create missing-ingredient notification:', notifErr.message);
      }
    }
