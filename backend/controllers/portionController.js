...
// Returns { quantity, unit } — if units are incompatible the quantity is kept
// in the recipe (from) unit so the plan still generates with correct display.
const convertToStandardUnit = (quantity, fromUnit, toUnit) => {
  ...
  const toGrams = {
    ...
    lb:453.59, oz:28.35, pound:453.59, pounds:453.59,
  };
  const fromGrams = {
    g:1, gram:1, grams:1,
    kg:0.001, kilogram:0.001, kilograms:0.001,
    lb:1/453.59, oz:1/28.35, pound:1/453.59, pounds:1/453.59,
  };
  ...
  const fromMl = {
    ml:1, l:0.001, liter:0.001, liters:0.001,
    cup:1/240, tbsp:1/15, tsp:1/5,
  };

  // Count base: individual units (pieces = 1)
  const toCount = {
  ...
  const isWeight = (u) => u in toGrams;
  const isVolume = (u) => u in toMl;
  const isCount  = (u) => u in toCount;

  // ── Count units ────────────────────────────────────────────────────────────
  // Count ↔ weight or count ↔ volume is physically meaningless.
  // Return quantity unchanged in the original (recipe) unit so the plan
  // still generates; we just can't convert across these categories.
  if (isCount(f) || isCount(t)) {
    if (isCount(f) && isCount(t)) {
      return quantity * toCount[f] * fromCount[t];
    }
    // Incompatible: keep recipe quantity as-is
    console.warn(`Unit conversion skipped: cannot convert "${f}" → "${t}" (count vs weight/volume)`);
    return quantity;
  }

  // ── Same category (weight↔weight or volume↔volume) ────────────────────────
  if (isWeight(f) && isWeight(t)) {
    return quantity * toGrams[f] * fromGrams[t];
  }
  if (isVolume(f) && isVolume(t)) {
    return quantity * toMl[f] * fromMl[t];
  }

  // ── Cross-category: weight ↔ volume ───────────────────────────────────────
  // Use water-density approximation (1 ml ≈ 1 g).
  // This is accurate for water/thin liquids; for spices it's an approximation
  // but far better than the silent × 1 fallback.
  if (isWeight(f) && isVolume(t)) {
    const inGrams = quantity * toGrams[f];   // to grams
    return inGrams * fromMl[t];              // grams ≈ ml → target volume
  }
  if (isVolume(f) && isWeight(t)) {
    const inMl = quantity * toMl[f];         // to ml
    return inMl * fromGrams[t];              // ml ≈ grams → target weight
  }

  // Unknown units — return as-is
  console.warn(`Unknown unit conversion: "${f}" → "${t}"`);
  return quantity;
};
