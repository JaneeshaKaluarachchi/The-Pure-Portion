...
const getTodayAttendance = async (req, res) => {
  try {
    ...
    // Get all staff
    const allStaff = await Staff.find({ restaurantId }, 'firstName lastName email position profileImage staffId');

    // Get today's attendance
    const todayAttendance = await Attendance.find({
      date: { $gte: today, $lt: tomorrow }
    }).populate('staffId', 'firstName lastName email position profileImage staffId');
