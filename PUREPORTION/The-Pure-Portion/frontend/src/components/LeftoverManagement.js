...
const LeftoverManagement = () => {
  ...
  const handleDownloadPDF = async (status = 'all', type = 'all') => {
    try {
      ...
      const response = await axios.get(`/api/leftovers/report/pdf?${params}`, {
        headers: { 
  ...
  const fetchLeftovers = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/leftovers?${params}`, {
  ...
  const handleHelpRequest = async (requestId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `/api/leftovers/requests/${requestId}/fulfill`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
  ...
  const fetchDonationRequests = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/leftovers/requests', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchMyLeftovers = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/leftovers/my-leftovers', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchPendingLeftovers = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/leftovers/admin/pending', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchPendingRequests = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/leftovers/admin/pending-requests', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchClaimNotifications = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/food-claim-notifications', {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      await Promise.all(leftoverIds.map(async (id) => {
        try {
          const response = await axios.get(
            `/api/food-claim-notifications/check-pending/${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
  ...
  const handleClaimSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('/api/food-claim-notifications/claim-request', {
  ...
  const markNotificationAsRead = async (notificationId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(
        `/api/food-claim-notifications/${notificationId}/read`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchClaimNotifications();
  ...
  const handleRespondToClaim = async (notificationId, action, rejectionReason = '') => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `/api/food-claim-notifications/${notificationId}/respond`,
  ...
  const handleDonateSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('/api/leftovers', formData, {
        headers: { 
          Authorization: `Bearer ${token}`,
  ...
  const handleRequestSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('/api/leftovers/request', formData, {
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'multipart/form-data'
        }
  ...
  const handleApproveReject = async (id, action, reason = '', type = 'leftover') => {
    try {
      ...
      const endpoint = type === 'leftover' 
        ? `/api/leftovers/admin/${id}/approve`
        : `/api/leftovers/admin/requests/${id}/approve`;
          ...
          <div className="leftovers-grid">
            {leftovers.map(leftover => (
              <div key={leftover._id} className="leftover-card">
                {leftover.imageUrl && (
                  <img src={`${leftover.imageUrl}`} alt={leftover.name} />
                )}
            ...
            <div className="details-content">
              {selectedDonation.imageUrl && (
                <div className="details-image">
                  <img src={`${selectedDonation.imageUrl}`} alt={selectedDonation.name} />
                </div>
              )}
