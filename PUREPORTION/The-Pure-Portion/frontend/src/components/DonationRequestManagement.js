...
const DonationRequestManagement = () => {
  ...
  const fetchDonationRequests = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/leftovers/requests?${params}`, {
  ...
  const fetchMyRequests = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/leftovers/requests?status=all', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchSoonToExpireItems = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/leftovers/soon-to-expire?days=7', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleCreateRequest = async (e) => {
    ...
    try {
      ...
      await axios.post('/api/leftovers/request', requestData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleFulfillRequest = async (e) => {
    ...
    try {
      ...
      await axios.post(`/api/leftovers/requests/${selectedRequest._id}/fulfill`, {
  ...
  const handleApproveReject = async (requestId, action, reason = '') => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`/api/leftovers/admin/requests/${requestId}/approve`, {
        action,
        reason
      }, {
        headers: { Authorization: `Bearer ${token}` }
