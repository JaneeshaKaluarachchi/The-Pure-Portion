...
const FinanceDashboard = () => {
  ...
  const fetchExecutedPortionPlans = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("/api/portions", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchFinanceData = async () => {
    try {
      ...
      const [dailyRes, monthlyRes, summaryRes, recordsRes] = await Promise.all([
        axios.get("/api/finance/daily-profit", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("/api/finance/monthly-profit", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("/api/finance/summary", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("/api/finance/records", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
                  ...
                  {staffList.map((staff) => (
                    ...
                    >
                      <div className="staff-photo-clean">
                        {staff.profileImage ? (
                          <img
                            src={`/uploads/staff-images/${staff.profileImage}`}
