...
const StaffFinanceManagement = ({ isModal = false }) => {
  ...
  const fetchPaymentHistory = async (staffId) => {
    try {
      ...
      const response = await axios.get(
        `/api/finance/staff-payments/${staffId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const generatePaymentReceipt = async (payment) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `/api/finance/payment-receipt/${payment._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
  ...
  const generateFullPaymentHistory = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `/api/finance/payment-history-pdf/${selectedStaff._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
        }
              ...
              <div className="profile-header-section">
                ...
                <div className="profile-photo-container">
                  {selectedStaff.profileImage ? (
                    <img
                      src={`/uploads/staff-images/${selectedStaff.profileImage}`}
